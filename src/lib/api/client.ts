...
function getSavedSearches(){
  if(savedSearchesCache && savedSearchesCache.expiresAt>Date.now()) return Promise.resolve(savedSearchesCache.data);
  if(savedSearchesInflight) return savedSearchesInflight;
  savedSearchesInflight=request<SavedSearchesResponse>("/saved-searches").then(data=>{savedSearchesCache={data,expiresAt:Date.now()+SAVED_SEARCH_CACHE_TTL_MS};return data}).finally(()=>{savedSearchesInflight=null});
