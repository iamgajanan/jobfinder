...
type ApiErrorDetail = string | { message?: string } | Array<{ msg?: string; message?: string }>;
type ApiErrorBody = { detail?: ApiErrorDetail; message?: string; error?: string };

function getErrorMessage(payload: ApiErrorBody | null, status: number) {
...
async function parseResponse<T>(response: Response): Promise<T> {
  ...
  throw new ApiError(response.status, getErrorMessage(payload as ApiErrorBody | null, response.status));
}
...
async function request<T>(path: string, init: RequestInit = {}, retryOnUnauthorized = true): Promise<T> {
  ...
  if (response.status === 401 && retryOnUnauthorized && session?.refresh_token) {
    try {
      const refreshed = await requestRefresh(session.refresh_token);
...
async function requestRefresh(refreshToken: string) {
  const response = await fetch("/api/backend/auth/refresh", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refresh_token: refreshToken }), cache: "no-store" });
  return parseResponse<NonNullable<AuthResponse["session"]>>(response);
}

const SAVED_SEARCH_CACHE_TTL_MS = 30_000;
let savedSearchesCache: { data: SavedSearchesResponse; expiresAt: number } | null = null;
let savedSearchesInflight: Promise<SavedSearchesResponse> | null = null;
...
async function getAccount() { try { const data = await request<AccountResponse>("/account/me"); accountCache = data; return data; } catch (error) { if (accountCache) return accountCache; throw error; } }
export const api = {
