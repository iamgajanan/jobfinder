...
import { ExternalLink, LoaderCircle, MapPin, Eye } from "lucide-react";
import { ApiError, api } from "@/lib/api/client";
import type { Job, ViewedJob } from "@/lib/api/types";
...
export default function ViewedJobs() {
  ...
  useEffect(() => {
    api.viewedJobs()
      .then((response) => setItems(response.viewed_jobs))
      .catch((err) => setError(err instanceof ApiError ? err.message : "Unable to load viewed jobs."))
  ...
  if (error) return <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-500">{error}</div>;

  return <section>
    <div className="mb-7">
      <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-3 py-1 text-xs font-semibold text-indigo-500"><Eye size={14}/> Activity</div>
      <h1 className="text-3xl font-bold md:text-4xl">Viewed jobs</h1>
      <p className="mt-2 text-[var(--muted)]">Jobs you opened from JobFinder. This does not mean you applied.</p>
    </div>
