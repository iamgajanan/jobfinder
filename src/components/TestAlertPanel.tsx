...
export default function TestAlertPanel() {
  ...
  async function loadHistory(id: string) {
    ...
    setHistoryError("");
    try {
      const [status, alertJobs] = await Promise.all([api.alertStatus(id), api.alertJobs(id, 10)]);
  ...
  return (
    <div className="mb-6 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="flex items-center gap-2 font-semibold"><BellRing size={17} /> Job alert test & history</div>
      ...
      <div className="mt-5 border-t border-[var(--border)] pt-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-[var(--muted)]">Next run: {formatDate(nextRunAt)} · Last run: {formatDate(lastRunAt)}</p>
          </div>
          {historyLoading && <LoaderCircle size={16} className="animate-spin text-indigo-500" />}
        </div>
        ...
        {!historyLoading && !historyError && (
          <>
            ...
            <div className="mt-4">
              <h4 className="text-sm font-semibold">Recently seen alert jobs</h4>
              {jobs.length === 0 ? (
                <p className="mt-2 text-xs text-[var(--muted)]">No jobs recorded for this alert yet.</p>
              ) : (
                <div className="mt-2 grid gap-2 md:grid-cols-2">
                  {jobs.slice(0, 6).map((item) => (
                    <a key={item.id} href={item.job_data.apply_url || item.job_data.job_url} target="_blank" rel="noreferrer" className="rounded-xl border border-[var(--border)] p-3 hover:border-indigo-500/40">
                      <p className="text-sm font-medium">{item.job_data.title}</p>
                      <p className="mt-1 text-xs text-[var(--muted)]">{item.job_data.company} · {item.job_data.location}</p>
                      <p className="mt-1 text-[11px] text-[var(--muted)]">First seen {formatDate(item.first_seen_at)}</p>
                    </a>
                  ))}
                </div>
              )}
